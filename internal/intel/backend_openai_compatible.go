...
func taskOutputContract(task string) string {
	...
	case TaskEndpointExtract:
		return `return JSON only with keys: selected_url:string, evidence_page_url:string, kind:string, confidence:number. selected_url must be one embedded URL from the provided candidate_pages embedded_urls list or one provided page_url. kind must be one of native_api_endpoint, compat_api_endpoint, docs_page, unrelated. confidence must be between 0 and 1.`
	default:
...
func contractForTask(task string) string {
	...
	case TaskEndpointExtract:
		return "For endpoint_extract, choose the most likely native provider endpoint or API base URL from the provided embedded URLs. Prefer first-party native endpoints over proxies, compatibility layers, billing pages, policy pages, and generic docs. Never invent a URL and never output a URL that is not present in the input."
	default:
		return ""
	}
}
