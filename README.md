...
7. End-to-end deterministic `Read`, `Query`, and `Crawl` orchestration in `internal/core/service`, with query strategies `off|same_site_links|web_search` and a multi-provider two-stage `web_search` path that merges bootstrap candidates, probes top pages, and expands same-host child pages before final selection
...
11. Golden end-to-end fixtures and benchmark coverage for `read`, `query`, and `crawl`, including comparison against naive and reduced deterministic baselines plus a measured external `trafilatura` adapter path
...
- [read-validation-2026-03-28.md](/home/jose/hpdev/Libraries/needlex/improvements/read-validation-2026-03-28.md)
